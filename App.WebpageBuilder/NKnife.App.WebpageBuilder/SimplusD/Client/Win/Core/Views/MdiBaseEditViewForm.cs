...
{
    public class MdiBaseEditViewForm : BaseEditViewForm, IMarkPosition
    {
        #region 字段与属性
        ...
        public MdiBaseEditViewForm(string id)
        {
            MarkPanel(id);
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            {
                //MyTip.HideTip();
                this.MainPanel.IsNewOpenForAddEvent = false;
            };
        ...
        void RegisterForm()
        {
            ...
            string configForm = Path.Combine(PathService.CL_Dialog_Folder, this.Name + ".xml");

            ///若是继承自BaseListManageForm，则先注册此类的xml
            if (this.GetType().BaseType.FullName == "BaseListManageForm")
            {
                string baseListConfigForm = Path.Combine(PathService.CL_Dialog_Folder, "BaseListManageForm.xml");
                XmlDocument baseListDocForm = new XmlDocument();
                baseListDocForm.Load(baseListConfigForm);

                ProcessXmlDocument(baseListDocForm);

                ///继承自BaseLIstManageForm的允许没有自己本身的窗体配置文件
        ...
        //绑定所有控件鼠标按下事件 by lisuye on 2008年6月5
        ...
        ////绑定所有控件焦点离开事件 by lisuye on 2008年6月5
        //void LeaveEvent(Control parent, EventHandler leaveHandler)
        //{
        //    foreach (Control control in parent.Controls)
        //    {
        //        if (control.HasChildren)
        //        {
        //            LeaveEvent(control, leaveHandler);
        //        }
            ...
            foreach (XmlNode node in _docForm.DocumentElement.ChildNodes)
            {
                ...
                if (attValidate != null)
                {
                    ...
                    if (!ValidateService.Validate(attValidate.Value, targetControl, out errorText))
                    {
                        ...
                        allErrorMsg.Append(node.Attributes["label"].Value)
                            .Append("-")
                            .Append(errorText).AppendLine();
                        isSuccess = false;
                    }
                }
            }
        ...
        {
            string value;
            if (!_textResources.TryGetValue(name, out value))
            {
                Debug.Fail("没取到值,name:" + name);
                value = string.Empty;
            }
            return value;
        }
        ...
        {
            XmlElement ele = Service.Sdsite.CurrentDocument.GetElementById(this.pageId);
            this.Text = ele.GetAttribute("title");
        }
        ...
        public override bool IsModified
        {
            get
            {
                if (this.MainPanel.IsNewOpenForAddEvent)
                {
                    return false;
                }
                else
        ...
        public override bool Save()
        {
            MainPanel.Save();
            PageDoc.Save();
            this.PageDoc = Service.Sdsite.CurrentDocument.GetPageDocumentById(this.PageId);
            MainPanel.FillValue(new[] { this.PageDoc });
            return true;
        }
        ...
        public PageXmlDocument PageDoc { get; private set; }
        private AutoLayoutPanel MainPanel { get; set; }

        private void MarkPanel(string id)
        {
            this.PageId = id;
            this.PageDoc = Service.Sdsite.CurrentDocument.GetPageDocumentById(id);
            this.MainPanel = AutoLayoutPanelEx.CreatePanel(typeof(EditorAttribute), this.PageDoc, false, true);
            this.Controls.Add(this.MainPanel);
            MainPanel.FillValue(new[] { this.PageDoc });
            this.MainPanel.Dock = DockStyle.Fill;
            Service.Sdsite.CurrentDocument.NodeChanged += new XmlNodeChangedEventHandler(CurrentDocument_NodeChanged);
        }
        //当属性面板的值进行改变的同时编辑form的值也随之改变 by lisuye on 2008年6月2日
        void CurrentDocument_NodeChanged(object sender, XmlNodeChangedEventArgs e)
        {
            this.PageDoc = Service.Sdsite.CurrentDocument.GetPageDocumentById(this.PageId);
            // string str=this.PageDoc.ToString();
            MainPanel.FillValue(new[] { this.PageDoc });
            this.MainPanel.IsNewOpenForAddEvent = true;
        }
        ...
        public void MarkPosition(Position position)
        {
            ...
            Object[] atts = property.GetCustomAttributes(typeof(EditorAttribute), false);
            if (atts != null)
            {
                EditorAttribute att = (EditorAttribute)atts[0];
                string leftLabelName = att.LabelLeft;
                if (!string.IsNullOrEmpty(leftLabelName))
                {
                    string result = GetValue(leftLabelName);
                    //找到对应的控件
                    Control control = ControlFind(this, result + ":");
                    if (control != null)
                    {
                        //得到他的下一个控件将查找的内容高亮显示
                        Control con = control.Parent.Controls[control.TabIndex + 1];
                        if (con is TextBox)
                        {
                            ((TextBox)con).Focus();
                            ((TextBox)con).SelectionStart = position.Index;
                            ((TextBox)con).SelectionLength = position.Length;
                        }
                        //TODO:HTML查找替换的高亮显示 lisye（未处理）
                        if (con is HTMLDesignControl)
                        {
                        }
                    }
                }
        ...
        /// <returns></returns>
        public Control ControlFind(Control parentControl, string result)
        {
            ...
            foreach (Control control in parentControl.Controls)
            {
                ...
                {
                    controlReturn = control;
                }
                if (control.HasChildren)
                {
                    Control tagControl = ControlFind(control, result);
                    if (tagControl != null)
                    {
                        return tagControl;
                    }
        ...
        {
            string path = Path.Combine(PathService.CHS_Folder, "AutoLayoutPanelResource.xml");
            XmlDocument xmlDoc = new XmlDocument();
            xmlDoc.Load(path);

            XmlNode node = xmlDoc.DocumentElement.SelectSingleNode("//text[@name='" + result + "']");
            string lableName = node.InnerText;
            return lableName;
        }
        ...
        public Position GetCurrentPosition()
        {
            ...
            PageXmlDocument doc = Service.Sdsite.CurrentDocument.GetPageDocumentById(this.Id);
            if (control is TextBox)
            {
                if (control.Parent is ValueControl)
                {
                    property = (PropertyInfo)(((ValueControl)control.Parent).MemberInfo);
                }
                int index = ((TextBox)control).SelectionStart;
                int length = ((TextBox)control).SelectionLength;
                position = new Position(index, length, property, doc);
                if (string.IsNullOrEmpty(FindOptions.Singler.FindContent))
                    FindOptions.Singler.FindContent = ((TextBox)control).Text.Substring(index, length);
                FindOptions.Singler.CurrentPosition = position;
            }
            //获得当前的节点 如果控件是HTML设计器（未处理）lisuye
            if (control is HTMLDesignControl)
            {
            }
            return position;
        }
