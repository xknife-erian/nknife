...
namespace WeifenLuo.WinFormsUI.Docking
{
	...
	public partial class DockPane : UserControl, IDockDragSource
	{
		...
		internal protected DockPane(IDockContent content, DockState visibleState, bool show)
		{
            ...
            ///我们手动添加的鼠标点击事件，为了使鼠标中键能关闭当前窗体，若更换版本，则仍需添加此段代码
            TabStripControl.MouseClick += new MouseEventHandler(TabStripControl_MouseClick);
		}

        void TabStripControl_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Middle)
            {
                this.ActiveContent.DockHandler.Close();
            }
		}
