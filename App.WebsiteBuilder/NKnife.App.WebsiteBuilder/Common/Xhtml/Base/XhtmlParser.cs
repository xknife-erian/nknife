...
namespace Jeelu
{
    ...
    /// <summary>
    /// 这是一个对外来的Xhtml节点片断进行解析的类型，
    /// 从HTML PARSER FOR .NET v1.4，Author: Andy Powney ( andy@powney.demon.co.uk ) 移植
    /// 2008年6月10日21时13分，未完成，暂不能使用
    /// </summary>
    public static class XhtmlParser
    {

        #region 对Xhtml节点片断进行解析的主要方法
        ...
        private static int FindTagOpenNodeIndex(XhtmlTCollection<XhtmlElement> nodes, string name)
        {
            for (int index = nodes.Count - 1; index >= 0; index--)
            {
                if (nodes[index] is XhtmlElement)
                {
                    if (((XhtmlElement)nodes[index]).LocalName.ToLower().Equals(name.ToLower())
                        && !((XhtmlElement)nodes[index]).HasChildElements
                        && ((XhtmlElement)nodes[index]).IsTerminated == false)
                    {
                        return index;
                    }
                }
        ...
        private static string PreprocessScript(string input, string tag_name)
        {
            StringBuilder output = new StringBuilder();
            int index = 0;
