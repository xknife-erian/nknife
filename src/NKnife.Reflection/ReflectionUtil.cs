...
namespace NKnife.Reflection
{
    public class ReflectionUtil
    {
        ...
        private static Delegate GetEventHandler(EventInfo eventInfo, object handlerHost, string handlerMethodName)
        {
            ...
            {
                if (eventInfo == null)
                    throw new Exception($"Unable to find an event named '{eventInfo.Name}'.");
                ...
                if (delegateParameters == null || delegateParameters.Length != 2)
                    throw new InvalidOperationException($"Event '{eventInfo.Name}' is not valid.");
                ...
                if (doEventMethod == null)
                    throw new Exception($"{handlerMethodName} method doesn't exist.");
                if (!doEventMethod.IsGenericMethod)
                    throw new Exception($"{handlerMethodName} method is not a generic method.");
