...
namespace NKnife.Chinese
{
    ...
    public class SbcAndDbc
    {
        ...
        /// </summary>
        private const int MaxSbc = 65374;

        /// <summary>最小的有效全角英文字符转换成int型数据的值
        /// </summary>
        private const int MinSbc = 65281;

        /// <summary>最大的有效半角英文字符转换成int型数据的值
        /// </summary>
        private const int MaxDbc = 126;

        /// <summary>最小的有效半角英文字符转换成int型数据的值
        /// </summary>
        private const int MinDbc = 33;
        ...
        public static char ToDbc(char originalChar)
        {
            ...
            if (originalChar == '　')
                return ' ';
            if (originalChar >= MinSbc && originalChar <= MaxSbc)
        ...
        public static char ToSbc(char originalChar)
        {
            ...
            if (originalChar == ' ')
                return '　';
            if (originalChar >= MinDbc && originalChar <= MaxDbc)
                return (char) (originalChar + Margin);
            return originalChar;
        }
