    ...
    public abstract class CloneableObject : ICloneable
    {
        ...
        public object Clone()
        {
            ...
            foreach (var fi in GetType().GetFields())
            {
                ...
                var cloneType = fi.FieldType.GetInterface(nameof(ICloneable), true);
                if (cloneType != null)
                {
                    //取得对象的ICloneable接口。                   
                ...
                var enumerableType = fi.FieldType.GetInterface(nameof(IEnumerable), true);
                if (enumerableType != null)
                {
                    //取得该字段的IEnumerable接口                  
                    var em = (IEnumerable) fi.GetValue(this);
                    var listType = fields[i].FieldType.GetInterface(nameof(IList), true);
                    var dicType = fields[i].FieldType.GetInterface(nameof(IDictionary), true);
                    var j = 0;
                    if (listType != null)
                    {
                        //取得IList接口。                       
                        ...
                        foreach (var obj in em)
                        {
                            ...
                            cloneType = obj.GetType().GetInterface(nameof(ICloneable), true);
                            if (cloneType != null)
                            {
                                //如果支持ICloneable 接口，               
                                //用它李设置列表中的对象的克隆             
                    ...
                    else if (dicType != null)
                    {
                        ...
                        {
                            //查看当前项是否支持支持ICloneable 接口。                           
                            cloneType = de.Value.GetType().GetInterface(nameof(ICloneable), true);
                            if (cloneType != null)
                            {
