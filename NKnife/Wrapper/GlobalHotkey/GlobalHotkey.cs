...
namespace Gean
{
    ...
    public static class GlobalHotKey
    {
        ...
        public static void Regist(IntPtr hWnd, API.User32.GlobalHotkeyModifiers fsModifiers, Keys vk, HotKeyCallBackHanlder callBack)
        {
            int id = keyid++;
            if (!API.User32.RegisterHotKey(hWnd, id, fsModifiers, vk))
        ...
        public static void UnRegist(IntPtr hWnd, HotKeyCallBackHanlder callBack)
        {
            foreach (KeyValuePair<int, HotKeyCallBackHanlder> var in keymap)
            {
                if (var.Value == callBack)
                    API.User32.UnregisterHotKey(hWnd, var.Key);
            }
        }
