...
namespace NKnife.Datas.DbHelper
{
    /// <summary>
    ///     SqlHelper是一个微软完全开放源码的数据库操作类, 用于简化你重复的去写那些数据库连接（DbConnection）,DbCommand,DbDataReader等等。
    ///     SqlHelper 封装过后通常是只需要给方法传入一些参数如数据库连接字符串，SQL参数等，就可以访问数据库。
    ///     SqlHelper 类用于通过一组静态方法来封装数据访问功能。
    ///     该类不能被继承，因此将其声明为包含专用构造函数的不可继承类。
