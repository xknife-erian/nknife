    ...
    public class WebClient
    {
        ...
        private void WriteFileField(string fileField)
        {
            ...
            foreach (string var in strArr)
            {
                ...
                fileField = "--" + BOUNDARY + "\r\n";
                fileField += "Content-Disposition: form-data; name=\"" + M.Groups[1].Value + "\"; filename=\"" +
        ...
        private void SendRequestData(string URL, string method, bool showProgress)
        {
            ...
            if (postStream != null)
            {
                var buffer = new byte[SEND_BUFFER_SIZE];
                int count = 0;
                Stream sm = clientSocket.GetStream();
                postStream.Position = 0;

                var e = new UploadEventArgs();
                ...
                do
                {
                    ...
                    if (showProgress)
                    {
                        //触发事件
                        e.bytesSent += count;
                        e.sendProgress = e.bytesSent/(double) e.totalBytes;
                        double t = timer.ElapsedMilliseconds/1000;
                        t = t <= 0 ? 1 : t;
                        e.sendSpeed = e.bytesSent/t;
        ...
        private void SaveNetworkStream(Stream toStream, bool showProgress)
        {
            ...
            do
            {
                ...
                if (showProgress)
                {
                    ...
                    e.ReceiveProgress = e.bytesReceived/(double) e.totalBytes;
                    ...
                    e.receiveSpeed = e.bytesReceived/t;
