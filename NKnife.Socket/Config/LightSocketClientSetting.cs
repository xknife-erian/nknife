...
namespace SocketKnife.Config
{
    public abstract class LightSocketClientSetting : XmlCoderSetting, ISocketClientSetting
    {
        ...
        protected override void Load(XmlElement source)
        {
            ...
            XmlNode node = source.SelectSingleNode("HeartRange");
            if (node != null)
                int.TryParse(node.InnerText, out heartRange);
            HeartRange = heartRange;

            int bufferSize = 2048;
            XmlNode xmlNode = source.SelectSingleNode("BufferSize");
            if (xmlNode != null)
            ...
            Decoder =
                (IDatagramDecoder) UtilityType.CreateSimpleObject(source.SelectSingleNode("IDatagramDecoder").InnerText);
            Encoder =
                (IDatagramEncoder) UtilityType.CreateSimpleObject(source.SelectSingleNode("IDatagramEncoder").InnerText);
            CommandParser =
                (IDatagramCommandParser)
                    UtilityType.CreateSimpleObject(source.SelectSingleNode("IDatagramCommandParser").InnerText);
        }
    }
}