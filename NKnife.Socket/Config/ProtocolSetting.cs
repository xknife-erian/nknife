...
namespace SocketKnife.Config
{
    public abstract class ProtocolSetting : XmlCoderSetting
    {
        ...
        private void ParseProtocolFamily(XmlElement source)
        {
            try
            {
                foreach (XmlNode famNode in source.ChildNodes)
                {
                    ...
                    //家族默认的Content类型
                    XmlNode contentNode = famElement.SelectSingleNode("Content");
                    if (contentNode == null)
                        continue;
                    string contentTxt = contentNode.InnerText.Trim();
                    Type contentType = UtilityType.FindType(contentTxt);
        ...
        /// <summary>
        ///     解析协议的工具接口集,如果未发现定义协议的工具,将使用家族的默认工具
        /// </summary>
        /// <param name="family">The family.</param>
